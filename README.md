...
| POST | `/v1/ijara-lease-fulfillment-arrangement/initiate` | Initiate |
| GET | `/v1/ijara-lease-fulfillment-arrangement` | Retrieve (list) |
| GET | `/v1/ijara-lease-fulfillment-arrangement/{crId}/retrieve` | Retrieve |
| PUT | `/v1/ijara-lease-fulfillment-arrangement/{crId}/update` | Update |
| PUT | `/v1/ijara-lease-fulfillment-arrangement/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/ijara-lease-fulfillment-arrangement/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t isb/isb-ijara-leasing:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install isb-ijara-leasing ./helm -n isb-financing
```

Exposed through the platform Gateway at path prefix `/isb-ijara-leasing` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
